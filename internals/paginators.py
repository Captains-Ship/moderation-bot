# ripped straight from my personal bot (Cap+)
# a bit scuffed, but it works:tm:
...
class BPV(discord.ui.View):
    ...
    @discord.ui.button(
        emoji="", custom_id=ButtonType.left2, style=discord.ButtonStyle.gray
    )
    async def left2(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id == self.ctx.author.id:
            await self.paginator._decrease(2, interaction)  # noqa

    @discord.ui.button(
        emoji="", custom_id=ButtonType.left1, style=discord.ButtonStyle.success
    )
    async def left1(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id == self.ctx.author.id:
            await self.paginator._decrease(1, interaction)  # noqa

    @discord.ui.button(
        emoji="Capstop:928210625677656074",
        custom_id=ButtonType.stop,
        style=discord.ButtonStyle.danger,
    )
    async def _stop(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id == self.ctx.author.id:
            await self.paginator.stop(interaction)

    @discord.ui.button(
        emoji="Capright:928210625669238794",
        custom_id=ButtonType.right1,
        style=discord.ButtonStyle.success,
    )
    async def right1(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id == self.ctx.author.id:
            await self.paginator._increase(1, interaction)  # noqa

    @discord.ui.button(
        emoji="Capright2:928210625652482048",
        custom_id=ButtonType.right2,
        style=discord.ButtonStyle.gray,
    )
    async def right2(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id == self.ctx.author.id:
            await self.paginator._increase(2, interaction)  # noqa
...
class ButtonPaginator:
    def __init__(
        ...
        *,
        ...
        self.buttons: dict = buttons or {
            "\U000025c0": ButtonType.left2,
            "\U00002b05": ButtonType.left1,
            "\U000023f9": ButtonType.stop,
            "\U000027a1": ButtonType.right1,
            "\U000025b6": ButtonType.right2,
        }
        self._stopped: bool = False
        self.view: BPV = BPV(
            ctx=self.ctx, buttons=self.buttons, paginator=self, timeout=self.timeout
        )
    ...
    async def _edit_message(self, interaction: discord.Interaction = None):
        ...
        if isinstance(self.pages[self.index], discord.Embed):
            ...
            if self.force_embed:
                x = discord.Embed(
                    title=self.title,
                    description=self.prefix + self.pages[self.index] + self.suffix,
                    color=self.color,
                )
                await self.msg.edit(content="", embed=x)
            else:
                await self.msg.edit(
                    content=self.prefix + self.pages[self.index] + self.suffix,
                    embed=None,
                )
    ...
    async def _decrease(self, amount: int, interaction: discord.Interaction):
        """
    ...
    async def _increase(self, amount: int, interaction: discord.Interaction):
        """
    ...
    async def _handle_sending(self):
        ...
        self.msg = await self.ctx.send(
            ...
            embed=tobesent if isinstance(tobesent, discord.Embed) else None,
            view=self.view,
        )